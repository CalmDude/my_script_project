...
def get_sp500_tickers():
    ...
    try:
        with urllib.request.urlopen(req) as response:
            ...
            print(f"âœ“ Loaded {len(tickers)} S&P 500 tickers\n")
            return tickers
    except Exception as e:
        print(f"âŒ Error fetching S&P 500 list: {e}")
...
def get_nasdaq100_tickers():
    ...
    try:
        with urllib.request.urlopen(req) as response:
            ...
            for table in tables:
                if 'Ticker' in table.columns:
                    ...
                    tickers = [t.replace('.', '-') for t in tickers]
                    print(f"âœ“ Loaded {len(tickers)} NASDAQ 100 tickers\n")
                    return tickers
        ...
        print(f"âŒ Error fetching NASDAQ 100 list: {e}")
...
def get_portfolio_tickers():
    ...
    try:
        ...
        if not stocks_file.exists():
            print(f"âŒ stocks.txt not found")
            return []
        ...
        print(f"âœ“ Loaded {len(tickers)} portfolio tickers from stocks.txt\n")
        return tickers
    except Exception as e:
        print(f"âŒ Error reading stocks.txt: {e}")
        return []
...
def scan_stocks(tickers, category="stocks", daily_bars=60, weekly_bars=52, concurrency=2):
    ...
    print(f"ðŸ” Scanning {total} {category} stocks for 'FULL HOLD + ADD' signals...")
            ...
            try:
                ...
                if 'error' not in result:
                    ...
                    if signal == 'FULL HOLD + ADD':
                        ...
                        print(f"âœ“ [{completed}/{total}] {ticker:6s} -> {signal:20s} ${price:.2f}")
    ...
    print("=" * 80)
    print(f"\nâœ“ Scan complete: {len(results)} analyzed, {len(buy_signals)} FULL HOLD + ADD signals found\n")

    # Rate limit warning
    if rate_limit_errors > 0:
        print(f"âš ï¸  WARNING: {rate_limit_errors} rate limit errors detected!")
        print(f"   Yahoo Finance may be blocking requests. Wait 5-10 minutes before next scan.\n")
    elif len(results) == 0 and total > 0:
        print(f"âš ï¸  WARNING: No results returned from {total} tickers!")
...
def create_portfolio_excel(all_df, output_file, category="Portfolio"):
    ...
    df_hold_reduce = all_df[all_df['signal'] == 'HOLD MOST â†’ REDUCE'][available_cols].copy()
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        # Sheet 3: HOLD MOST â†’ REDUCE (summary in PDF)
        df_hold_reduce.to_excel(writer, sheet_name='HOLD MOST â†’ REDUCE', index=False)
    ...
    print(f"  - HOLD MOST â†’ REDUCE: {len(df_hold_reduce)} stocks")
...
def create_pdf_report(buy_df, all_results_df, output_file, timestamp_str, category=""):
    ...
    hold_reduce_df = source_df[source_df['signal'] == 'HOLD MOST â†’ REDUCE'].sort_values('ticker') if is_portfolio else pd.DataFrame()
    ...
    â€¢ Total {report_title_display} stocks analyzed: {total_stocks}<br/>
    â€¢ FULL HOLD + ADD signals found: {len(buy_signals_df)} ({len(buy_signals_df)/total_stocks*100:.1f}%)<br/>
    <br/>
    <b>Report Contents:</b><br/>
    â€¢ <b>{len(buy_signals_df)} FULL HOLD + ADD</b> - Maximum bullish alignment (detailed analysis)<br/>
    """

    if is_portfolio:
        cover_text += f"""
    â€¢ <b>{len(hold_reduce_df)} HOLD MOST â†’ REDUCE</b> - Weakening momentum (summary)<br/>
    â€¢ <b>{len(hold_df)} HOLD</b> - Neutral trend (summary)<br/>
    â€¢ <b>{len(cash_df)} CASH</b> - Bearish trend (summary)<br/>
        """

    cover_text += f"""
    <br/>
    <b>Each FULL HOLD + ADD stock includes:</b><br/>
    âœ“ Entry zones using Support levels (S1, S2, S3)<br/>
    âœ“ Upside targets using Resistance levels (R1, R2, R3)<br/>
    âœ“ Risk management with stops 5% below S3<br/>
    âœ“ Trailing stop guidelines for profit taking<br/>
    <br/>
    ...
    if not buy_signals_df.empty:
        ...
        for _, row in buy_signals_df.iterrows():
            ...
            â€¢ <b>Zone 1 (Aggressive):</b> ${s1:,.2f} (S1 - Near current price)<br/>
            â€¢ <b>Zone 2 (Patient):</b> ${s2:,.2f} (S2 - Stronger support)<br/>
            â€¢ <b>Zone 3 (Deep Value):</b> ${s3:,.2f} (S3 - Major support)<br/>
            <br/>
            ...
            if nan_count > 0:
                ...
                <br/>
                <b>Note:</b> â­ N/A means limited overhead resistance - stock breaking into new highs!<br/>
            ...
            tech_data = [
                ...
                ['R3', 'Clear Skies â­' if pd.isna(r3) else f"${r3:,.2f}", 'Major Resistance', 'Full exit target'],
                ['R2', 'Clear Skies â­' if pd.isna(r2) else f"${r2:,.2f}", 'Key Resistance', 'Take 50% profits'],
                ['R1', 'Clear Skies â­' if pd.isna(r1) else f"${r1:,.2f}", 'Minor Resistance', 'Take 25% profits'],
            ...
            â€¢ <b>1st tranche (25% at R1):</b> 8-10% trailing stop - Lock in quick gains<br/>
            â€¢ <b>2nd tranche (25% at R2):</b> 12-15% trailing stop - Room for volatility<br/>
            â€¢ <b>Core position (50%):</b> 15-20% trailing stop - Let winners run<br/>
            <br/>
        ...
        # SECTION 2: HOLD MOST â†’ REDUCE
        if not hold_reduce_df.empty:
            elements.append(Paragraph(f"HOLD MOST â†’ REDUCE ({len(hold_reduce_df)} stocks)", section_style))
            elements.append(Spacer(1, 0.1*inch))
            ...
            # Summary table for HOLD MOST â†’ REDUCE
    ...
    # Build PDF
    doc.build(elements)
    print(f"âœ“ PDF report created: {output_file}")
    if is_portfolio:
        print(f"  - {len(buy_signals_df)} FULL HOLD + ADD (detailed)")
        print(f"  - {len(hold_reduce_df)} HOLD MOST â†’ REDUCE (summary)")
