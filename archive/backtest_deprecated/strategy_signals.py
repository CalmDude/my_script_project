...
class GHBStrategy:
    ...
    def calculate_support_distance(
        self, df: pd.DataFrame, close: float
    ) -> Tuple[float, float]:
        """
        Calculate support level and distance from support

        Matches notebook logic:
        - Support = 60-day low (recent swing low)
        - Distance = (close - support) / close * 100

        Returns:
            Tuple of (support_level, distance_percentage)
        """
        # Use 60-day low as support (recent swing low)
        if len(df) < 60:
            # Fallback: use available data
            support = df["Low"].min()
        else:
            support = df["Low"].iloc[-60:].min()

        if close == 0:
            return support, 0.0

        # Distance formula from notebook: (close - support) / close * 100
        distance = ((close - support) / close) * 100
    ...
    def calculate_signals(self, df: pd.DataFrame, ticker: str = None) -> Dict:
        ...
        # Calculate support level and distance from support (NEW for scanner logic)
        support, to_support_pct = self.calculate_support_distance(df, close)
    ...
    def filter_safe_entries(
        self, df_signals: pd.DataFrame, max_distance: float = 10.0
    ) -> pd.DataFrame:
        """
        Filter P1 buy signals to only include safe entries (within max distance from support)

        This implements the scanner's entry risk filtering logic:
        - Only buy stocks within max_distance% of support
        - Maintains momentum-first sorting (like base GHB strategy)

        Args:
            df_signals: DataFrame with all signals
            max_distance: Maximum distance from support (default 10%)

        Returns:
            Filtered DataFrame with safe P1 entries, sorted by momentum
        """
        p1_signals = df_signals[df_signals["State"] == "P1"].copy()

        if p1_signals.empty:
            return p1_signals

        # Filter by distance from support
        safe_entries = p1_signals[p1_signals["To_Support_%"] < max_distance].copy()

        # Sort by MOMENTUM first (like base strategy), maintaining best opportunities
        # This is the key fix: prioritize momentum, not distance
        safe_entries = safe_entries.sort_values(["ROC_4W_%"], ascending=[False])
